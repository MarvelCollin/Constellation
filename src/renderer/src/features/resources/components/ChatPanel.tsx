...
export function ChatPanel({ busy, disabled, error, onRefresh, onSend, snapshot, title }: ChatPanelProps) {
  ...
  return (
    <section className="chat-panel" aria-labelledby={`${title.replace(/\s+/g, "-").toLowerCase()}-heading`}>
      <div className="chat-header">
        <div>
          <span className={disabled ? "scan-state" : "scan-state scan-state-ready"}>
            {disabled ? "Chat offline" : "Chat online"}
          </span>
          <h2 id={`${title.replace(/\s+/g, "-").toLowerCase()}-heading`}>{title}</h2>
          <p>Token-protected messages for nodes connected to this server.</p>
        </div>
